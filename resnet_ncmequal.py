...
class NCMResModel(rn.ResNetX):
  ...
  def __init__(self, resnet_size, bottleneck, num_classes, num_filters,
    ...
    self.total = tf.get_variable("total",[num_classes,num_classes],dtype=tf.float32,trainable=False, initializer=tf.initializers.constant(0))
  ...
  def __call__(self, inputs, training):
    ...
    if self.ncmmethod == "softmax":
        ...
        deepx = tf.layers.dense(inputs=deepx, units=self.num_classes)
