...
def resnet_model_fn(features, labels, mode, model_class,
                    ...
                    data_format, version, loss_filter_fn=None, multi_gpu=False, ncmmethod=deepncm.NCM_DEFAULT_METHOD):
  ...
  model = model_class(resnet_size, data_format, version=version,ncmmethod=ncmmethod)
  logits, deep_x = model(features, mode == tf.estimator.ModeKeys.TRAIN)
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    train_op = tf.group(optimizer.minimize(loss, global_step), ncm_ops, update_ops)
  else:
    train_op = None
...
def resnet_main(flags, model_function, input_function, shape=None):
  ...
  run_config = tf.estimator.RunConfig().replace(
     ...
     save_summary_steps = 100,       # Number of steps between summaries
...
class ResnetArgParser(argparse.ArgumentParser):
  ...
  def __init__(self, resnet_size_choices=None):
    ...
    self.add_argument(
        ...
        default=deepncm.RESNET_DEFAULT_VERSION,
    ...
    self.add_argument(
        '--ncmmethod', '-n', default=deepncm.NCM_DEFAULT_METHOD,
